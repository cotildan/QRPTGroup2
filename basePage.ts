...
interface Options{
    driver?: WebDriver;
    /* if no driver is supplied, we have one or use the constructor
    to make one*/

    url?: string;
    /* if no url is supplied, we have one or use one given here*/
}
...
export class BasePage{
    ...
    async navigate(url?: string): Promise<void> {
        ...
        return Promise.reject(
            'mainBase.navigate() needs a url defined on the page object or one pased in'
        )
    }
